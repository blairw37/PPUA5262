...
census_indicators<-read.csv('Module11/CensusIndicators/DEC_CENSUS_2010_TRACT.csv')
...
craigslist_listings$YES_SECTION_8 <- craigslist_listings$SECTION_8 - craigslist_listings$NO_SECTION_8
...
ct_listing_quality_index<-craigslist_listings %>%
  group_by(CT_ID_10) %>%
  summarise(AVG_LQI=mean(LISTING_QUALITY_INDEX))
...
tracts<-merge(uhi, ct_stats, by='CT_ID_10', all.x = TRUE)

lm(AVG_LQI~CAN_CT, data=tracts)

reg<-lm(AVG_LQI~CAN_CT, data=tracts)
summary(reg)

# Calculating standardized betas with lm.beta
lm.beta(reg)
cor.test(tracts$CAN_CT,tracts$AVG_LQI)

base<-ggplot(data=tracts, aes(x=CAN_CT, y=AVG_LQI)) +
  geom_point() + xlab("% Canopy Coverage") + ylab("Listing Quality Index")
base

base+geom_smooth(method=lm)

# Categorical independent variable
tracts<-merge(tracts, redlining, by='CT_ID_10', all.x = TRUE)
reg_grade<-lm(AVG_LQI~Grade, data=tracts)
summary(reg_grade)
